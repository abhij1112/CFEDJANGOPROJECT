...
from .forms import RecipeForm,RecipeIngredientsForm,RecipeIngredintsImageForm
...
@login_required
def recipe_ingrdient_update_hx_view(request,parent_id=None, id=None):
    if not request.htmx:
        raise Http404
    try:
...
def recipe_ingredient_image_upload_view(request, parent_id = None):
    ...
    if parent_obj is None:
        raise Http404
    form = RecipeIngredintsImageForm(request.POST or None, request.FILES or None)
    if form.is_valid():
        obj = form.save(commit=False)
